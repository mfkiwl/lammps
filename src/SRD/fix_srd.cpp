    ...
    rbuf2(nullptr), nbinbig(nullptr), binbig(nullptr), binsrd(nullptr), stencil(nullptr)
{
  ...
  for (int ishift = 0; ishift < 2; ishift++)
    for (int iswap = 0; iswap < 6; iswap++)
      shifts[ishift].bcomm[iswap].sendlist = shifts[ishift].bcomm[iswap].recvlist = nullptr;
  ...
  if (bigexist) {
    ...
    while (i < nall) {
      if (mask[i] & biggroupbit) {
        ...
        if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y || iz < 0 || iz >= nbin2z)
        ...
        for (j = 0; j < nstencil; j++) {
          ...
          if (jx < 0 || jx >= nbin2x || jy < 0 || jy >= nbin2y || jz < 0 || jz >= nbin2z)
            error->one(FLERR,
                       "Fix SRD: bad stencil bin for big particle\n"
                       "Big particle {} {} {:.8} {:.8} {:.8}\n"
                       "Bin indices: {} {} {}, {} {} {}, {} {} {}\n",
                       atom->tag[i], i, x[i][0], x[i][1], x[i][2], ix, iy, iz, jx, jy, jz, nbin2x,
                       nbin2y, nbin2z);
        ...
        for (ix = lo; ix <= hi; ix++)
          for (iy = 0; iy < nbin2y; iy++)
            for (iz = 0; iz < nbin2z; iz++) {
              ibin = iz * nbin2y * nbin2x + iy * nbin2x + ix;
              if (nbinbig[ibin] == ATOMPERBIN) error->all(FLERR, "Fix SRD: too many walls in bin");
...
void FixSRD::post_force(int /*vflag*/)
{
  ...
  for (i = nlocal; i < nall; i++) f[i][0] = f[i][1] = f[i][2] = 0.0;

  if (collidestyle == NOSLIP)
    for (i = nlocal; i < nall; i++) torque[i][0] = torque[i][1] = torque[i][2] = 0.0;
      ...
      if (mask[i] & groupbit) {
        ...
        if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y || iz < 0 || iz >= nbin2z)
          error->one(FLERR,
                     ...
                     "v = {:.8} {:.8} {:.8}\nx =  {:.8} {:.8} {:.8}\n"
                     "ix,iy,iz nx,ny,nz = {} {} {} {} {} {}\n",
                     atom->tag[i], update->ntimestep, v[i][0], v[i][1], v[i][2], x[i][0], x[i][1],
                     x[i][2], ix, iy, iz, nbin2x, nbin2y, nbin2z);
      }
  } else {
    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
  ...
  for (i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      if (x[i][0] < srdlo_reneigh[0] || x[i][0] > srdhi_reneigh[0] || x[i][1] < srdlo_reneigh[1] ||
          x[i][1] > srdhi_reneigh[1] || x[i][2] < srdlo_reneigh[2] || x[i][2] > srdhi_reneigh[2])
        flag = 1;
    }
  ...
  if (wallexist) {
    for (m = 0; m < nwall; m++)
      if (fabs(xwall[m] - xwallhold[m]) > walltrigger) next_reneighbor = update->ntimestep + 1;
  }
...
void FixSRD::vbin_comm(int ishift)
{
  ...
  for (int idim = 0; idim < dimension; idim++) {
    ...
    if (procgrid[idim] == 1) {
      if (bcomm1->nsend) vbin_pack(vbin, bcomm1->nsend, bcomm1->sendlist, sbuf1);
      if (bcomm2->nsend) vbin_pack(vbin, bcomm2->nsend, bcomm2->sendlist, sbuf2);
      if (bcomm1->nrecv) vbin_unpack(sbuf1, vbin, bcomm1->nrecv, bcomm1->recvlist);
      if (bcomm2->nrecv) vbin_unpack(sbuf2, vbin, bcomm2->nrecv, bcomm2->recvlist);

    } else {
      if (bcomm1->nrecv)
        MPI_Irecv(rbuf1, bcomm1->nrecv * VBINSIZE, MPI_DOUBLE, bcomm1->recvproc, 0, world,
                  &request1);
      if (bcomm2->nrecv)
        MPI_Irecv(rbuf2, bcomm2->nrecv * VBINSIZE, MPI_DOUBLE, bcomm2->recvproc, 0, world,
                  &request2);
      if (bcomm1->nsend) {
        vbin_pack(vbin, bcomm1->nsend, bcomm1->sendlist, sbuf1);
        MPI_Send(sbuf1, bcomm1->nsend * VBINSIZE, MPI_DOUBLE, bcomm1->sendproc, 0, world);
      }
      if (bcomm2->nsend) {
        vbin_pack(vbin, bcomm2->nsend, bcomm2->sendlist, sbuf2);
        MPI_Send(sbuf2, bcomm2->nsend * VBINSIZE, MPI_DOUBLE, bcomm2->sendproc, 0, world);
      }
      if (bcomm1->nrecv) {
...
void FixSRD::xbin_comm(int ishift, int nval)
{
  ...
  for (int idim = 0; idim < dimension; idim++) {
    ...
    if (procgrid[idim] == 1) {
      if (bcomm1->nsend) xbin_pack(vbin, bcomm1->nsend, bcomm1->sendlist, sbuf1, nval);
      if (bcomm2->nsend) xbin_pack(vbin, bcomm2->nsend, bcomm2->sendlist, sbuf2, nval);
      if (bcomm1->nrecv) xbin_unpack(sbuf1, vbin, bcomm1->nrecv, bcomm1->recvlist, nval);
      if (bcomm2->nrecv) xbin_unpack(sbuf2, vbin, bcomm2->nrecv, bcomm2->recvlist, nval);

    } else {
      if (bcomm1->nrecv)
        MPI_Irecv(rbuf1, bcomm1->nrecv * nval, MPI_DOUBLE, bcomm1->recvproc, 0, world, &request1);
      if (bcomm2->nrecv)
        MPI_Irecv(rbuf2, bcomm2->nrecv * nval, MPI_DOUBLE, bcomm2->recvproc, 0, world, &request2);
      if (bcomm1->nsend) {
        xbin_pack(vbin, bcomm1->nsend, bcomm1->sendlist, sbuf1, nval);
        MPI_Send(sbuf1, bcomm1->nsend * nval, MPI_DOUBLE, bcomm1->sendproc, 0, world);
      }
      if (bcomm2->nsend) {
        xbin_pack(vbin, bcomm2->nsend, bcomm2->sendlist, sbuf2, nval);
        MPI_Send(sbuf2, bcomm2->nsend * nval, MPI_DOUBLE, bcomm2->sendproc, 0, world);
      }
      if (bcomm1->nrecv) {
        MPI_Wait(&request1, MPI_STATUS_IGNORE);
...
void FixSRD::xbin_pack(BinAve *vbin, int n, int *list, double *buf, int nval)
{
  ...
  for (int i = 0; i < n; i++) {
    j = list[i];
    for (k = 0; k < nval; k++) buf[m++] = vbin[j].value[k];
  }
}
...
void FixSRD::xbin_unpack(double *buf, BinAve *vbin, int n, int *list, int nval)
{
  ...
  for (int i = 0; i < n; i++) {
    j = list[i];
    for (k = 0; k < nval; k++) vbin[j].value[k] += buf[m++];
  }
}
    ...
    while (collide_flag) {
      ...
      for (m = 0; m < nbig; m++) {
        ...
        if (inside) {
          if (exactflag) {
            if (type == SPHERE)
              t_remain = collision_sphere_exact(x[i], x[j], v[i], v[j], big, xscoll, xbcoll, norm);
            else if (type == ELLIPSOID)
              t_remain =
      ...
      for (m = 0; m < nbig; m++) {
        ...
        if (inside) {
          if (type == SPHERE)
            ...
            t_remain = collision_ellipsoid_exact(x[i], x[j], v[i], v[j], big, xscoll, xbcoll, norm);
          else if (type == LINE)
...
int FixSRD::inside_ellipsoid(double *xs, double *xb, Big *big)
{
  ...
  if (x * x * big->aradsqinv + y * y * big->bradsqinv + z * z * big->cradsqinv <= 1.0) return 1;
  return 0;
}
...
int FixSRD::inside_line(double *xs, double *xb, double *vs, double *vb, Big *big, double dt_step)
{
...
int FixSRD::inside_tri(double *xs, double *xb, double *vs, double *vb, Big *big, double dt_step)
{
  ...
  if (a == 0.0) {
    ...
    if (0.0 <= root1 && root1 <= 1.0)
      tfraction = root1;
    else if (0.0 <= root2 && root2 <= 1.0)
      tfraction = root2;
    else
      error->one(FLERR, "Bad quadratic solve for particle/tri collision");
  }
...
double FixSRD::collision_sphere_exact(double *xs, double *xb, double *vs, double *vb, Big *big,
...
void FixSRD::collision_sphere_inexact(double *xs, double *xb, Big *big, double *xscoll,
...
double FixSRD::collision_ellipsoid_exact(double *xs, double *xb, double *vs, double *vb, Big *big,
                                         double *xscoll, double *xbcoll, double *norm)
{
  ...
  a = (bx * bx + 2.0 * ax * cx) * big->aradsqinv + (by * by + 2.0 * ay * cy) * big->bradsqinv +
      (bz * bz + 2.0 * az * cz) * big->cradsqinv;
  b = 2.0 * (bx * cx * big->aradsqinv + by * cy * big->bradsqinv + bz * cz * big->cradsqinv);
  c = cx * cx * big->aradsqinv + cy * cy * big->bradsqinv + cz * cz * big->cradsqinv - 1.0;
...
void FixSRD::collision_ellipsoid_inexact(double *xs, double *xb, Big *big, double *xscoll,
                                         double *xbcoll, double *norm)
{
  ...
  double scale =
      1.0 / sqrt(x * x * big->aradsqinv + y * y * big->bradsqinv + z * z * big->cradsqinv);
  x *= scale;
  y *= scale;
  z *= scale;
...
double FixSRD::collision_line_exact(double * /*xs*/, double * /*xb*/, double * /*vs*/,
                                    double * /*vb*/, Big * /*big*/, double dt_step, double *xscoll,
                                    double *xbcoll, double *norm)
{
  xscoll[0] = xsc[0];
  xscoll[1] = xsc[1];
...
double FixSRD::collision_tri_exact(double * /*xs*/, double * /*xb*/, double * /*vs*/,
                                   double * /*vb*/, Big * /*big*/, double dt_step, double *xscoll,
                                   double *xbcoll, double *norm)
{
  xscoll[0] = xsc[0];
...
double FixSRD::collision_wall_exact(double *xs, int iwall, double *vs, double *xscoll,
                                    double *xbcoll, double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
void FixSRD::collision_wall_inexact(double *xs, int iwall, double *xscoll, double *xbcoll,
                                    double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
void FixSRD::slip(double *vs, double *vb, double *xb, Big *big, double *xsurf, double *norm,
                  double *vsnew)
{
...
void FixSRD::slip_wall(double *vs, int iwall, double *norm, double *vsnew)
{
  ...
  scale =
      1.0 / sqrt(tangent1[0] * tangent1[0] + tangent1[1] * tangent1[1] + tangent1[2] * tangent1[2]);
  tangent1[0] *= scale;
  tangent1[1] *= scale;
  tangent1[2] *= scale;
...
void FixSRD::noslip(double *vs, double *vb, double *xb, Big *big, int iwall, double *xsurf,
...
void FixSRD::force_torque(double *vsold, double *vsnew, double *xs, double *xb, double *fb,
                          double *tb)
{
...
int FixSRD::update_srd(int i, double dt, double *xscoll, double *vsnew, double *xs, double *vs)
{
  int ix, iy, iz;
  ...
  if (xs[0] < srdlo[0] || xs[0] > srdhi[0] || xs[1] < srdlo[1] || xs[1] > srdhi[1] ||
...
void FixSRD::parameterize()
{
  ...
  if (any_ellipsoids == 0 && any_lines == 0 && any_tris == 0 && collidestyle == SLIP)
    torqueflag = 0;
  else
    torqueflag = 1;
  ...
  if (flagall) error->all(FLERR, "Fix srd requires SRD particles all have same mass");
  ...
  if (lamdaflag == 0)
    ...
    temperature_srd = force->mvv2e * (lamda / dt_srd) * (lamda / dt_srd) * mass_srd / force->boltz;
  ...
  if (dimension == 3)
    viscosity =
        gridsrd * gridsrd / (18.0 * dt_srd) * (1.0 - (1.0 - exp(-srd_per_cell)) / srd_per_cell) +
        (force->boltz * temperature_srd * dt_srd / (4.0 * mass_srd * force->mvv2e)) *
            ((srd_per_cell + 2.0) / (srd_per_cell - 1.0));
  else
    viscosity = (force->boltz * temperature_srd * dt_srd / (2.0 * mass_srd * force->mvv2e)) *
  ...
  if (comm->me == 0) {
    ...
    mesg += fmt::format("  SRD temperature & lamda = {:.8} {:.8}\n", temperature_srd, lamda);
    mesg += fmt::format("  SRD max distance & max velocity = {:.8} {:.8}\n", dmax, vmax);
    mesg += fmt::format("  SRD grid counts: {} {} {}\n", nbin1x, nbin1y, nbin1z);
    mesg += fmt::format("  SRD grid size: request, actual (xyz) = {:.8}, {:.8} {:.8} {:.8}\n",
  ...
  if (nbin1x < comm->procgrid[0] || nbin1y < comm->procgrid[1] || nbin1z < comm->procgrid[2])
  ...
  int tolflag = 0;
  if (binsize3y / binsize3x > 1.0 + cubictol || binsize3x / binsize3y > 1.0 + cubictol) tolflag = 1;
  if (dimension == 3) {
    if (binsize3z / binsize3x > 1.0 + cubictol || binsize3x / binsize3z > 1.0 + cubictol)
      tolflag = 1;
  }
  ...
  tolflag = 0;
  if (binsize3x / gridsrd > 1.0 + cubictol || gridsrd / binsize3x > 1.0 + cubictol) tolflag = 1;
  if (binsize3y / gridsrd > 1.0 + cubictol || gridsrd / binsize3y > 1.0 + cubictol) tolflag = 1;
  if (dimension == 3) {
    if (binsize3z / gridsrd > 1.0 + cubictol || gridsrd / binsize3z > 1.0 + cubictol) tolflag = 1;
  }
  ...
  for (int k = 0; k < nbig; k++) {
    ...
    if (biglist[k].type == SPHERE) {
      ...
      MathExtra::angmom_to_omega(angmom[i], biglist[k].ex, biglist[k].ey, biglist[k].ez, inertia,
                                 biglist[k].omega);
      MathExtra::matvec(biglist[k].ex, biglist[k].ey, biglist[k].ez, biglist[k].normbody,
                        biglist[k].norm);
...
void FixSRD::setup_bounds()
{
  ...
  if (bigexist) {
    ...
    if (triclinic == 0) {
      ...
      if (dimension == 3) subsize = MIN(subsize, domain->prd[2] / comm->procgrid[2]);
    } else {
      subsize = 1.0 / comm->procgrid[0] / length0;
      subsize = MIN(subsize, 1.0 / comm->procgrid[1] / length1);
      if (dimension == 3) subsize = MIN(subsize, 1.0 / comm->procgrid[2] / length2);
    }
    dist_srd_reneigh = subsize - onemove;
  }
...
void FixSRD::setup_velocity_shift(int ishift, int dynamic)
{
  ...
  if (ishift == 0) {
    if (myloc[0] * nbin1x % procgrid[0] == 0) binlo[0] = myloc[0] * nbin1x / procgrid[0];
    if (myloc[1] * nbin1y % procgrid[1] == 0) binlo[1] = myloc[1] * nbin1y / procgrid[1];
    if (myloc[2] * nbin1z % procgrid[2] == 0) binlo[2] = myloc[2] * nbin1z / procgrid[2];
  ...
  if (ishift == 0) {
    if (myloc[0] * nbin1x % procgrid[0] == 0) first->nsend = second->nrecv = 0;
    if ((myloc[0] + 1) * nbin1x % procgrid[0] == 0) second->nsend = first->nrecv = 0;
  } else {
  ...
  if (ishift == 0) {
    if (myloc[1] * nbin1y % procgrid[1] == 0) first->nsend = second->nrecv = 0;
    if ((myloc[1] + 1) * nbin1y % procgrid[1] == 0) second->nsend = first->nrecv = 0;
  } else {
  ...
  if (dimension == 3) {
    ...
    if (ishift == 0) {
      if (myloc[2] * nbin1z % procgrid[2] == 0) first->nsend = second->nrecv = 0;
      if ((myloc[2] + 1) * nbin1z % procgrid[2] == 0) second->nsend = first->nrecv = 0;
    } else {
  ...
  if (dynamic == 0 && nbins > shifts[ishift].maxvbin) {
    ...
    shifts[ishift].vbin = (BinAve *) memory->smalloc(nbins * sizeof(BinAve), "fix/srd:vbin");
  }
...
double FixSRD::newton_raphson(double t1, double t2)
{
  ...
  for (int i = 0; i < MAXITER; i++) {
    if ((((t - thi) * df - f) * ((t - tlo) * df - f) > 0.0) || (fabs(2.0 * f) > fabs(dtold * df))) {
      dtold = dt;
      dt = 0.5 * (thi - tlo);
      t = tlo + dt;
      if (tlo == t) return t;
