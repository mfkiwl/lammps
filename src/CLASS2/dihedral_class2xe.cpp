...
DihedralClass2xe::DihedralClass2xe(LAMMPS *lmp) : Dihedral(lmp)
{
...
DihedralClass2xe::~DihedralClass2xe()
{
  if (copymode) return;
  ...
  if (allocated) {
    ...
    memory->destroy(bb13t_d0);
    memory->destroy(bb13t_alpha);
...
void DihedralClass2xe::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    if (fabs(bb13t_d0[type]) > SMALL) {
      ...
      tk1 = -bb13t_d0[type]*bb13t_alpha[type]*bb13_ralpha1*(1 - bb13_ralpha2)*dr1/r3;
      tk2 = -bb13t_d0[type]*bb13t_alpha[type]*bb13_ralpha2*(1 - bb13_ralpha1)*dr2/r1;
...
void DihedralClass2xe::allocate()
{
  allocated = 1;
  ...
  memory->create(mbt_alpha2,n+1,"dihedral:mbt_alpha2");

  memory->create(ebt_alpha1,n+1,"dihedral:ebt_alpha1");
  ...
  memory->create(ebt_alpha3,n+1,"dihedral:ebt_alpha3");
  ...
  memory->create(bb13t_d0,n+1,"dihedral:bb13t_d0");
  memory->create(bb13t_alpha,n+1,"dihedral:bb13t_alpha");
...
void DihedralClass2xe::coeff(int narg, char **arg)
{
  ...
  if (strcmp(arg[1],"mbt") == 0) {
    ...
    double f1_one = utils::numeric(FLERR,arg[3],false,lmp);
    double f2_one = utils::numeric(FLERR,arg[4],false,lmp);
    double f3_one = utils::numeric(FLERR,arg[5],false,lmp);
    double r0_one = utils::numeric(FLERR,arg[6],false,lmp);

    for (int i = ilo; i <= ihi; i++) {
      mbt_alpha2[i] = alpha_one;
      mbt_f1[i] = f1_one;
      mbt_f2[i] = f2_one;
      mbt_f3[i] = f3_one;
      mbt_r0[i] = r0_one;
    ...
    double alpha1_one = utils::numeric(FLERR,arg[2],false,lmp);
    double alpha3_one = utils::numeric(FLERR,arg[3],false,lmp);
    double f1_1_one = utils::numeric(FLERR,arg[4],false,lmp);
    double f2_1_one = utils::numeric(FLERR,arg[5],false,lmp);
    double f3_1_one = utils::numeric(FLERR,arg[6],false,lmp);
    double f1_2_one = utils::numeric(FLERR,arg[7],false,lmp);
    double f2_2_one = utils::numeric(FLERR,arg[8],false,lmp);
    double f3_2_one = utils::numeric(FLERR,arg[9],false,lmp);
    double r0_1_one = utils::numeric(FLERR,arg[10],false,lmp);
    double r0_2_one = utils::numeric(FLERR,arg[11],false,lmp);

    for (int i = ilo; i <= ihi; i++) {
      ebt_alpha1[i] = alpha1_one;
      ebt_alpha3[i] = alpha3_one;
      ebt_f1_1[i] = f1_1_one;
      ebt_f2_1[i] = f2_1_one;
      ebt_f3_1[i] = f3_1_one;
      ebt_f1_2[i] = f1_2_one;
    ...
    double r10_one = utils::numeric(FLERR,arg[4],false,lmp);
    double r30_one = utils::numeric(FLERR,arg[5],false,lmp);

    for (int i = ilo; i <= ihi; i++) {
      bb13t_d0[i] = d0_one;
      bb13t_alpha[i] = alpha_one;
      bb13t_r10[i] = r10_one;
      bb13t_r30[i] = r30_one;
      setflag_bb13t[i] = 1;
      count++;
...
void DihedralClass2xe::write_restart(FILE *fp)
{
  ...
  fwrite(&mbt_alpha2[1],sizeof(double),atom->ndihedraltypes,fp);
  ...
  fwrite(&ebt_alpha1[1],sizeof(double),atom->ndihedraltypes,fp);
  fwrite(&ebt_alpha3[1],sizeof(double),atom->ndihedraltypes,fp);
  ...
  fwrite(&bb13t_d0[1],sizeof(double),atom->ndihedraltypes,fp);
  fwrite(&bb13t_alpha[1],sizeof(double),atom->ndihedraltypes,fp);
...
void DihedralClass2xe::read_restart(FILE *fp)
{
  allocate();
  ...
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR,&mbt_alpha2[1],sizeof(double),atom->ndihedraltypes,fp,nullptr,error);
    ...
    utils::sfread(FLERR,&ebt_alpha1[1],sizeof(double),atom->ndihedraltypes,fp,nullptr,error);
    utils::sfread(FLERR,&ebt_alpha3[1],sizeof(double),atom->ndihedraltypes,fp,nullptr,error);
    ...
    utils::sfread(FLERR,&bb13t_d0[1],sizeof(double),atom->ndihedraltypes,fp,nullptr,error);
    utils::sfread(FLERR,&bb13t_alpha[1],sizeof(double),atom->ndihedraltypes,fp,nullptr,error);
  ...
  MPI_Bcast(&ebt_alpha1[1],atom->ndihedraltypes,MPI_DOUBLE,0,world);
  MPI_Bcast(&ebt_alpha3[1],atom->ndihedraltypes,MPI_DOUBLE,0,world);
  ...
  MPI_Bcast(&bb13t_d0[1],atom->ndihedraltypes,MPI_DOUBLE,0,world);
  MPI_Bcast(&bb13t_alpha[1],atom->ndihedraltypes,MPI_DOUBLE,0,world);
...
void DihedralClass2xe::write_data(FILE *fp)
{
  ...
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g %g %g %g %g\n",i,mbt_alpha2[i],mbt_f1[i],mbt_f2[i],mbt_f3[i],mbt_r0[i]);

  fprintf(fp,"\nBondBond13 Coeffs\n\n");
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g %g %g %g\n",i,bb13t_d0[i],bb13t_alpha[i],bb13t_r10[i],bb13t_r30[i]);
