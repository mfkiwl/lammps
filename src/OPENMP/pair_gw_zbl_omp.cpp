...
PairGWZBLOMP::PairGWZBLOMP(LAMMPS *lmp) : PairGWZBL(lmp), ThrOMP(lmp, THR_PAIR)
{
...
void PairGWZBLOMP::compute(int eflag, int vflag)
{
  ev_init(eflag, vflag);
...
void PairGWZBLOMP::eval(int iifrom, int iito, ThrData *const thr)
{
  ...
  for (int ii = iifrom; ii < iito; ++ii) {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      repulsive_gw(&params[iparam_ij], rsq, fpair, EFLAG, evdwl);
...
double PairGWZBLOMP::memory_usage()
{
  double bytes = memory_usage_thr();
  bytes += PairGWZBL::memory_usage();
  return bytes;
}
